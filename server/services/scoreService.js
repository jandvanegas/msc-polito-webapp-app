...
function scoreService(dao) {
  const evaluate = async (letter, category, level, words) => {
    ...
    if (passed) {
      ...
      const rounds = await dao.listLastRoundsByLetterAndCategory(letter, category, configData.NUMBER_OF_ROUNDS_TO_CHECK);
      ...
      const fullPoints = fullPointWords.length * configData.FULL_POINTS_PER_WORD * level;

      const halfPointWords = validWords.filter(word => {
        return lastUsedWords.includes(word) && word[0].toUpperCase() === letter
      })
      const halfPoints = halfPointWords.length * configData.HALF_POINTS_PER_WORD * level;

      const score = fullPoints + halfPoints;
      return {score: score, full_point_words: fullPointWords, half_point_words: halfPointWords, passed: passed}
    }
    return {score: 0, full_point_words: [], half_point_words: [], passed: passed};
  }
